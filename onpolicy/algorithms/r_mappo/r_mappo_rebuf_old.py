...
class R_MAPPO():
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        if self.rebuf_in is not None:
            ...
            # print(f"l1 replay loss: {replay_loss_l1}")

            #l2 Replay Loss #TODO!!
            replay_loss_l2 = torch.sum(replay_diff ** 2, dim=-1, keepdim=True).mean()

            # print(f"l2 replay loss: {replay_loss_l2}")
    ...
    def train(self, buffer, update_actor):
        ...
        # if self.rebuf_in is not None:
        #     loss_data['ppo_loss'] += ppo_loss.item()
        #     loss_data['l1_rebuf_loss'] += replay_loss_l1.item()
        #     loss_data['l2_rebuf_loss'] += replay_loss_l2.item()
        #     loss_data['overall_loss'] += policy_loss.item()

        #     for j in loss_data[k].keys():
